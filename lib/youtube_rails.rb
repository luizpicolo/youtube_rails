class YouTubeRails
  ...
  URL_FORMATS = {
      regular: %r{^(www\.)?youtube\.com/watch\?(.*\&)?v=(?<id>[^&]+)}i,
      shortened: %r{^(www\.)?youtu\.be/(?<id>[^&]+)}i,
      embed: %r{^(www\.)?youtube\.com/embed/(?<id>[^&]+)}i,
      embed_as3: %r{^(www\.)?youtube\.com/v/(?<id>[^?]+)}i,
      chromeless_as3: %r{^(www\.)?youtube\.com/apiplayer\?video_id=(?<id>[^&]+)}i,
  }
